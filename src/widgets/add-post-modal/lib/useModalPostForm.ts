...
import { addTweetSchema } from './addPostModal.schema';
import { useModalPostImages } from './useModalPostImages';

type useModalPostFormProps = {
  userId: string;
  onSuccess?: () => void;
};

export const useModalPostForm = ({
  userId,
  onSuccess,
}: useModalPostFormProps) => {
  const {
    previews,
    error: imageError,
    imageFiles,
